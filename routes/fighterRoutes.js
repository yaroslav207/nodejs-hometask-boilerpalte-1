...
router.get('/:id', async (req, res, next) => {
    try {
        if(!FighterService.hasInDB({id: req.params.id})){
            res.status(404)
...
router.post('/', createFighterValid, async (req, res, next) => {
    try {
        ...
        const checkCandidate = await FighterService.hasInDB({name: req.body.name})
        if (checkCandidate) {
            res.status(400)
            throw new Error('fighter with such an name already exists')
        }
